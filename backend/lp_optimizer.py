...
nuclear_available_gw :float =0.0 ,
    ...
    for t in techs :
        ...
        avail_twh =(existing_gw +nuclear_available_gw if t =="nuclear"else existing_gw )*p ["cf"]*8760 /1000 


        fuel_cost_twh =p ["fuel_cost_gj"]*p ["heat_rate_gj_mwh"]*1000 
        var_cost_twh =(p ["vopex_mwh"]+fuel_cost_twh )*1_000_000 


        ann_capex =p ["capex_mw"]*(DISCOUNT_RATE /(1 -(1 +DISCOUNT_RATE )**-p ["lifetime_yr"]))
        fixed_cost_gw_yr =(ann_capex +p ["fopex_mw_yr"])*1000 
    ...
    prob +=pulp .lpSum (gen [t ]for t in re_techs )>=renewables_target *pulp .lpSum (gen [t ]for t in techs ),"RE_Target"
    ...
    prob +=cap_new ["hydro"]<=1.0 ,"Hydro_Limit"
